...
void BackendHandler::GlfwWindowResizedCallback(GLFWwindow* window, int width, int height)
{
	...
	for (int i = 0; i < Application::Instance().scenes.size(); i++)
	{
		...
		Application::Instance().scenes[i]->Registry().view<GrainEffect>().each([=](GrainEffect& buf)
			{
				buf.Reshape(width, height);
			});
		Application::Instance().scenes[i]->Registry().view<PixelateEffect>().each([=](PixelateEffect& buf)
			{
				buf.Reshape(width, height);
			});
		Application::Instance().scenes[i]->Registry().view<GBuffer>().each([=](GBuffer& buf)
			{
				buf.Reshape(width, height);
			});
		Application::Instance().scenes[i]->Registry().view<IlluminationBuffer>().each([=](IlluminationBuffer& buf)
			{
				buf.Reshape(width, height);
			});
	}
